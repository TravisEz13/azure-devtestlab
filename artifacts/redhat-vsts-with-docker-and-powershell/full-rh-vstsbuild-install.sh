#!/bin/sh
# my usage <url> <pat> PkgEs <agentname> dexxadmin
# orignially from
# https://github.com/Azure/azure-quickstart-templates/blob/master/vsts-fullbuild-redhat-vm/scripts/full-rh-vstsbuild-install.sh
...
then 
    ...
    IFS='.' read -ra hostNameParts <<< "$fullHostName" 
    AGENT=${hostNameParts[0]}
fi
